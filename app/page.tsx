...
function addDaysToDateKey(dateKey: string, offsetDays: number) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) return todayDateKey();
...
function eventsOverlap(a: CalendarEventItem, b: CalendarEventItem) {
  if (a.date !== b.date) return false;
  const aStart = timeToMinutes(a.time);
  const bStart = timeToMinutes(b.time);
  if (aStart === null || bStart === null) return false;
  const aEnd = timeToMinutes(a.endTime) ?? aStart + 60;
  const bEnd = timeToMinutes(b.endTime) ?? bStart + 60;
  return aStart < bEnd && bStart < aEnd;
}

function eventTypeColor(type?: string) {
  const match = EVENT_TYPES.find((item) => item.name === type);
  return match?.color || '#2563eb';
}

const EVENT_TYPES = [
  { name: 'Court', color: '#38bdf8' },
  { name: 'Client', color: '#84cc16' },
  { name: 'Attorney', color: '#a855f7' },
  { name: 'Biometrics', color: '#f97316' },
  { name: 'Office/Staff', color: '#22c55e' },
  { name: 'ICE', color: '#ef4444' },
  { name: 'USCIS', color: '#f87171' },
  { name: 'Consult', color: '#94a3b8' },
  { name: 'Jail Call', color: '#b45309' },
  { name: 'Deadline', color: '#facc15' },
];

const WORKFLOW_PRESETS = [
  {
    name: 'Trial evidence workflow',
    description: 'Trial date plus evidence deadline, client evidence deadline, and prep call.',
    lawyerTasks: ['Calendar trial date', 'File and serve evidence packet', 'Review client evidence', 'Prepare witness/exhibit outline'],
    clientTasks: ['Upload all evidence and documents', 'Confirm witness names and contact information', 'Attend preparation call'],
  },
  {
    name: 'Felony case workflow',
    description: 'Criminal defense workflow with discovery, client proof, motions, and hearing prep.',
    lawyerTasks: ['Request discovery', 'Review police report and video', 'Identify motion issues', 'Prepare hearing/trial checklist'],
    clientTasks: ['Upload court notices', 'Upload bond paperwork', 'Send witness names', 'Confirm availability for attorney call'],
  },
  {
    name: 'Misdemeanor case workflow',
    description: 'Streamlined criminal matter workflow for hearings, mitigation, and client documents.',
    lawyerTasks: ['Review charging document', 'Check next court date', 'Prepare plea/trial recommendation', 'Calendar deadlines'],
    clientTasks: ['Upload ticket/complaint', 'Upload proof of classes or treatment', 'Confirm contact information'],
  },
  {
    name: 'Family law document workflow',
    description: 'Client documents, lawyer checklist, motion packet prep, and proposed order prep.',
    lawyerTasks: ['Review prior orders', 'Review financial documents', 'Prepare motion packet', 'Prepare proposed order'],
    clientTasks: ['Upload prior orders', 'Upload messages/photos', 'Upload income and expense proof', 'Complete client timeline'],
  },
];
...
export default function Home() {
  ...
  const [calendarForm, setCalendarForm] = useState(() => ({
    ...
    aiNotes: '',
    privateEvent: false,
    addTrialWorkflow: false,
  }));
  const [tasks, setTasks] = useState<TaskItem[]>([]);
  const [workspaceStorageReady, setWorkspaceStorageReady] = useState(false);
  const [taskForm, setTaskForm] = useState({ title: '', dueDate: '', reminder: '1 day before', notes: '', assignedTo: '', ownerType: 'lawyer' as 'lawyer' | 'client', caseName: '', clientName: '', priority: 'Medium' as 'Low' | 'Medium' | 'High', workflowName: '' });
  const [caseSubTab, setCaseSubTab] = useState<'intake' | 'info' | 'calendar' | 'documents' | 'tasks' | 'notes' | 'billing' | 'workflows' | 'tools'>('intake');
  const [adminSubTab, setAdminSubTab] = useState<'source' | 'users' | 'audit'>('source');
  const [intakeForm, setIntakeForm] = useState({ name: '', address: '', phone: '', email: '', issue: '', preferredContact: 'Phone', matterType: 'General', notes: '' });
  const [caseRecords, setCaseRecords] = useState<CaseRecord[]>([]);
  const [selectedCaseId, setSelectedCaseId] = useState('');
  const [caseNote, setCaseNote] = useState('');
  const [workflowChoice, setWorkflowChoice] = useState(WORKFLOW_PRESETS[0].name);
  ...
  const activeCase = useMemo(() => caseRecords.find((item) => item.id === selectedCaseId) || null, [caseRecords, selectedCaseId]);
  ...
  const filteredCalendarEvents = useMemo(() => calendarEvents.filter((item) => {
    const userMatches = calendarUserFilter === 'all' || item.assignedUserId === calendarUserFilter;
    const typeMatches = calendarEventTypeFilter === 'all' || item.eventType === calendarEventTypeFilter;
    return userMatches && typeMatches;
  }), [calendarEvents, calendarUserFilter, calendarEventTypeFilter]);
  const calendarEventsByDate = useMemo(() => {
    const grouped: Record<string, CalendarEventItem[]> = {};
    filteredCalendarEvents.forEach((item) => {
      if (!item.date) return;
  ...
  const selectedCalendarEvent = useMemo(() => calendarEvents.find((item) => item.id === selectedCalendarEventId) || null, [calendarEvents, selectedCalendarEventId]);
  ...
  const canManageUsers = (me?.user?.role === 'owner' || me?.user?.role === 'admin' || demoBypass) && workspaceMode !== 'tester';
  ...
  useEffect(() => {
    ...
    try {
      ...
      setCaseRecords(JSON.parse(window.localStorage.getItem(`firstChairCases:${workspaceStorageSuffix}`) || '[]'));
    } catch {
      setCalendarEvents([]);
      setTasks([]);
      setCaseRecords([]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && workspaceStorageReady) window.localStorage.setItem(`firstChairCases:${workspaceStorageSuffix}`, JSON.stringify(caseRecords));
  }, [caseRecords, workspaceStorageReady, workspaceStorageSuffix]);
...
NOTE: File was attached in chat. For full vault search/indexing, also upload it in Source Files.`);
  ...
  function assignedUserLabel(userId?: string) {
    if (!userId) return me?.user?.displayName || me?.user?.email || 'Unassigned';
    const user = firmUsers.find((item) => item.user_id === userId);
    return user?.display_name || user?.email || userId;
  }

  function calendarConflictFor(draft: CalendarEventItem) {
    return calendarEvents.find((item) => item.id !== draft.id && item.assignedUserId === draft.assignedUserId && eventsOverlap(item, draft));
  }

  function addGeneratedTrialDeadlines(baseEvent: CalendarEventItem) {
    const clientName = baseEvent.clientName || 'Client';
    const matter = baseEvent.matterName || baseEvent.title;
    const created: CalendarEventItem[] = [
      {
        ...baseEvent,
        id: `event-${Date.now()}-trial`,
        eventType: baseEvent.eventType || 'Court',
        brief: baseEvent.brief || `Trial/hearing event for ${matter}.`,
        aiNotes: baseEvent.aiNotes || 'AI note: verify the court notice, local rules, witness deadlines, and exhibit service deadline before relying on this schedule.',
      },
      {
        id: `event-${Date.now()}-evidence-file`,
        title: 'Evidence due to court / opposing party',
        date: addDaysToDateKey(baseEvent.date, -30),
        time: baseEvent.time || '09:00',
        reminder: '1 week before',
        eventType: 'Deadline',
        assignedUserId: baseEvent.assignedUserId,
        assignedUserName: baseEvent.assignedUserName,
        clientName,
        matterName: matter,
        notes: `Auto-created from trial workflow for ${matter}. File/serve evidence by this date unless court order/local rules say otherwise.`,
        brief: 'Evidence filing/service deadline generated 30 days before trial.',
        aiNotes: 'AI generated note: confirm exact evidence deadline against the court notice, scheduling order, and local court rule.',
      },
      {
        id: `event-${Date.now()}-client-evidence`,
        title: 'Client evidence due to firm',
        date: addDaysToDateKey(baseEvent.date, -60),
        time: '10:00',
        reminder: '1 week before',
        eventType: 'Client',
        assignedUserId: baseEvent.assignedUserId,
        ...
        clientName,
        matterName: matter,
        notes: 'Client homework: upload documents, photos, messages, receipts, witness names, and any other evidence requested by the firm.',
        brief: 'Client evidence deadline generated 60 days before trial.',
        aiNotes: 'AI generated note: send the client a document checklist. Text message should be vague: “You have items to complete in First Chair.”',
      },
      {
        id: `event-${Date.now()}-prep-call`,
        title: 'Client evidence/prep call',
        date: addDaysToDateKey(baseEvent.date, -90),
        time: '10:00',
        reminder: '1 day before',
        eventType: 'Client',
        assignedUserId: baseEvent.assignedUserId,
        ...
        clientName,
        matterName: matter,
        notes: 'Discuss what evidence the client has, explain upload homework, and confirm timeline for packet builder.',
        brief: 'Prep call generated 90 days before trial.',
        aiNotes: 'AI generated note: the call should result in client and lawyer task checklists.',
      },
    ];
    setCalendarEvents((current) => [...created, ...current].sort((a, b) => `${a.date} ${a.time || ''}`.localeCompare(`${b.date} ${b.time || ''}`)));
    addWorkflowTasksFromPreset('Trial evidence workflow', clientName, matter, baseEvent.date);
    setStatus('Trial workflow added: trial, evidence deadline, client evidence deadline, prep call, and matching lawyer/client tasks. Review and adjust dates if needed.');
  }
  ...
  function addCalendarEvent() {
    ...
    const assignedUserName = assignedUserLabel(calendarForm.assignedUserId);
    const eventItem: CalendarEventItem = {
      ...
      reminder: calendarForm.reminder,
      notes: calendarForm.notes,
      eventType: calendarForm.eventType,
      color: eventTypeColor(calendarForm.eventType),
      assignedUserId: calendarForm.assignedUserId,
      assignedUserName,
      clientName: calendarForm.clientName,
      matterName: calendarForm.matterName,
      location: calendarForm.location,
      meetingLink: calendarForm.meetingLink,
      brief: calendarForm.brief || `${calendarForm.eventType} event for ${calendarForm.clientName || 'the client'}${calendarForm.matterName ? ` / ${calendarForm.matterName}` : ''}.`,
      aiNotes: calendarForm.aiNotes || 'AI generated note: verify deadlines, service rules, and required follow-up tasks before relying on this event.',
      privateEvent: calendarForm.privateEvent,
    };
    const conflict = calendarConflictFor(eventItem);
    if (conflict && !window.confirm(`Scheduling conflict: ${assignedUserName} already has “${conflict.title}” on ${conflict.date} at ${conflict.time || 'no time'}. Add this event anyway?`)) return;
    if (calendarForm.addTrialWorkflow) {
      addGeneratedTrialDeadlines(eventItem);
    } else {
      setCalendarEvents((current) => [eventItem, ...current].sort((a, b) => `${a.date} ${a.time || ''}`.localeCompare(`${b.date} ${b.time || ''}`)));
      setStatus(conflict ? 'Calendar event added with a noted scheduling conflict.' : 'Calendar event added.');
    }
    setSelectedCalendarDate(eventDate);
    setCalendarMonth(monthKeyFromDateKey(eventDate));
    setSelectedCalendarEventId(eventItem.id);
    setCalendarForm((current) => ({ ...current, title: '', time: '', endTime: '', notes: '', brief: '', aiNotes: '', meetingLink: '', location: '', addTrialWorkflow: false }));
  }
  ...
  function addTask() {
    ...
    const taskItem: TaskItem = {
      ...
      ownerType: taskForm.ownerType,
      caseName: taskForm.caseName,
      clientName: taskForm.clientName,
      priority: taskForm.priority,
      workflowName: taskForm.workflowName,
    };
    setTasks((current) => [taskItem, ...current]);
    setTaskForm({ title: '', dueDate: '', reminder: '1 day before', notes: '', assignedTo: '', ownerType: 'lawyer', caseName: '', clientName: '', priority: 'Medium', workflowName: '' });
    setStatus('Task added.');
  }

  function addWorkflowTasksFromPreset(workflowName: string, clientName = intakeForm.name || 'Client', caseName = matterName || intakeForm.matterType || 'Case', anchorDate = todayDateKey()) {
    const preset = WORKFLOW_PRESETS.find((item) => item.name === workflowName) || WORKFLOW_PRESETS[0];
    const lawyerTasks = preset.lawyerTasks.map((title, index) => ({
      id: `task-${Date.now()}-lawyer-${index}`,
      title,
      dueDate: addDaysToDateKey(anchorDate, -Math.max(7, (index + 1) * 7)),
      reminder: '1 day before',
      status: 'open' as const,
      notes: `Auto-created from ${preset.name}. Adjust as needed before assigning.`,
      assignedTo: me?.user?.displayName || me?.user?.email || 'Attorney',
      ownerType: 'lawyer' as const,
      caseName,
      clientName,
      priority: index === 0 ? 'High' as const : 'Medium' as const,
      workflowName: preset.name,
    }));
    const clientTasks = preset.clientTasks.map((title, index) => ({
      id: `task-${Date.now()}-client-${index}`,
      title,
      dueDate: addDaysToDateKey(anchorDate, -Math.max(14, (index + 2) * 10)),
      reminder: '1 week before',
      status: 'open' as const,
      notes: 'Client-facing homework. Email can include the checklist; SMS should be vague and only say there are items to complete in First Chair.',
    ...
    setTasks((current) => [...lawyerTasks, ...clientTasks, ...current]);
    setStatus(`${preset.name} added with lawyer checklist and client homework checklist.`);
  }

  function addCustomWorkflowTasks() {
    const lines = customWorkflowText.split('\n').map((line) => line.trim()).filter(Boolean);
    if (!lines.length) {
      setStatus('Type or dictate workflow steps first. Put each step on its own line.');
      return;
    }
    const clientName = intakeForm.name || checklistClientName || 'Client';
    const caseName = matterName || checklistMatterName || intakeForm.matterType || 'Case';
    const generated = lines.map((line, index) => ({
      id: `task-${Date.now()}-custom-${index}`,
      title: line.replace(/^[-*\d.\s]+/, ''),
      dueDate: addDaysToDateKey(todayDateKey(), index * 3),
      reminder: '1 day before',
      status: 'open' as const,
      notes: 'Custom workflow step. Edit priority, assignee, and due date as needed.',
      assignedTo: index % 2 === 0 ? (me?.user?.displayName || 'Attorney') : clientName,
      ownerType: index % 2 === 0 ? 'lawyer' as const : 'client' as const,
      caseName,
      clientName,
      priority: 'Medium' as const,
    ...
    setCustomWorkflowText('');
    setStatus('Custom workflow steps were converted into editable tasks.');
  }

  function startDictation(target: 'workflow' | 'chat' | 'notes') {
    ...
    if (!SpeechRecognition) {
      setStatus('Microphone dictation is not supported in this browser. Use Chrome or Edge on HTTPS, or type the text instead.');
      return;
    }
    try {
      ...
      const commitTranscript = (rawTranscript: string) => {
        const transcript = cleanTranscript(rawTranscript);
        if (!transcript || committed) return;
        committed = true;
        if (target === 'chat') {
          setChatInput((current) => `${current}${current.trim() ? ' ' : ''}${transcript}`);
          setStatus('Voice captured for chat. Review it, then press Enter or Send.');
        } else if (target === 'notes') {
          setCaseNote((current) => {
            const nextValue = `${current}${current.trim() ? '\n' : ''}${transcript}`;
            if (activeCase) setCaseRecords((records) => records.map((item) => item.id === activeCase.id ? { ...item, notes: nextValue, updatedAt: new Date().toISOString() } : item));
            return nextValue;
          });
          setStatus('Voice note captured once. Review/edit the case note before relying on it.');
        } else {
          setCustomWorkflowText((current) => `${current}${current ? '\n' : ''}${transcript}`);
          setStatus('Voice workflow step captured once. Review/edit it, then create tasks.');
        }
      };
      recognition.onstart = () => {
        setIsDictating(true);
        setStatus(target === 'chat' ? 'Listening for chat message...' : target === 'notes' ? 'Listening for case note...' : 'Listening for workflow step...');
      };
      recognition.onresult = (event: any) => {
        ...
        latestInterim = cleanTranscript(interimText) || latestInterim;
        const finalTranscript = cleanTranscript(finalText);
        if (finalTranscript) {
      ...
      recognition.onerror = (event: any) => {
        const reason = event?.error ? `Microphone error: ${event.error}.` : 'Could not capture microphone audio.';
        setStatus(`${reason} Check browser microphone permission, use Chrome/Edge, or type the text instead.`);
      };
      recognition.onend = () => {
        if (!committed && latestInterim) commitTranscript(latestInterim);
        setIsDictating(false);
      };
      recognition.start();
    } catch {
      setIsDictating(false);
      setStatus('Could not start microphone dictation. Check browser permission, then try again.');
    }
  }

  function startWorkflowVoice() {
    startDictation('workflow');
  }
  ...
  function startNotesVoice() {
    startDictation('notes');
  }
  ...
  function selectCase(record: CaseRecord) {
    ...
    setMatterName(record.matterName);
    setChecklistClientName(record.clientName);
    setChecklistMatterName(record.matterName);
    ...
    setTaskForm((current) => ({ ...current, clientName: record.clientName, caseName: record.matterName }));
    setCaseSubTab('info');
    setStatus(`Opened case: ${record.matterName}.`);
  }

  function startNewCaseIntake() {
    setSelectedCaseId('');
    setIntakeForm({ name: '', address: '', phone: '', email: '', issue: '', preferredContact: 'Phone', matterType: 'General', notes: '' });
    setCaseNote('');
    setCaseSubTab('intake');
    setStatus('Started a new intake. Save it to create and open the case workspace tabs.');
  }

  function saveIntakeAsCase() {
    const now = new Date().toISOString();
    const generatedMatterName = intakeForm.issue ? `${intakeForm.name || 'New client'} - ${intakeForm.issue.slice(0, 60)}` : `${intakeForm.name || 'New client'} - ${intakeForm.matterType} matter`;
    const nextRecord: CaseRecord = {
      id: selectedCaseId || `case-${Date.now()}`,
      clientName: intakeForm.name || 'New client',
      matterName: generatedMatterName,
      matterType: intakeForm.matterType || 'General',
      issue: intakeForm.issue || '',
      phone: intakeForm.phone || '',
      email: intakeForm.email || '',
      address: intakeForm.address || '',
      status: 'open',
      createdAt: activeCase?.createdAt || now,
      updatedAt: now,
      notes: intakeForm.notes || caseNote || '',
    };
    setMatterForm((current) => ({
      ...current,
      client_name: nextRecord.clientName || current.client_name,
      client_contact_name: nextRecord.clientName || current.client_contact_name,
      client_contact_email: nextRecord.email || current.client_contact_email,
      matter_name: nextRecord.matterName,
      matter_type: nextRecord.matterType,
      description: nextRecord.issue || nextRecord.notes,
      client_goals: nextRecord.issue || current.client_goals,
    }));
    setCaseRecords((current) => {
      const existingIndex = current.findIndex((item) => item.id === nextRecord.id);
      if (existingIndex >= 0) return current.map((item) => item.id === nextRecord.id ? nextRecord : item);
      return [nextRecord, ...current];
    });
    setSelectedCaseId(nextRecord.id);
    setMatterName(nextRecord.matterName);
    setChecklistClientName(nextRecord.clientName);
    setChecklistMatterName(nextRecord.matterName);
    setTaskForm((current) => ({ ...current, clientName: nextRecord.clientName, caseName: nextRecord.matterName }));
    setCaseSubTab('info');
    setStatus('Case created and opened. Its Intake, Info, Calendar, Documents, Tasks, Notes, Billing, Workflows, and Tools tabs are now available.');
  }

  function disableUserWithConfirm(userRow: FirmUserRow) {
    if (!window.confirm(`Disable ${userRow.display_name || userRow.email}? They will be treated as inactive in this workspace.`)) return;
    void updateFirmUser(userRow, { status: 'disabled' });
  }
  ...
  const navItems: [string, string][] = [
    ...
    ['tasks', '✅ Tasks'],
    ['billing', '💳 Billing'],
    ...(canManageUsers ? ([['admin', '⚙️ Admin']] as [string, string][]) : []),
  ];
    ...
    <main className="app-shell">
      ...
      {activeTab === 'cases' && (
        <section className="case-workspace">
          <section className="case-dashboard grid">
            <div className="card wide-card">
              <div className="section-label">Case list</div>
              <h2>Create or open a case</h2>
              <p className="subtle small">Start with intake, then click a case to open its Info, Calendar, Documents, Tasks, Notes, Billing, Workflows, and Tools tabs.</p>
              <div className="btn-row dense"><button onClick={startNewCaseIntake}>+ New case intake</button><button className="secondary" onClick={() => setCaseSubTab('intake')}>Edit current intake</button></div>
              <div className="case-list-grid">
                {caseRecords.length === 0 && <div className="item small subtle">No local cases yet. Fill out the intake form and click Save intake to case workspace.</div>}
                {caseRecords.map((record) => (
                  <button type="button" key={record.id} className={selectedCaseId === record.id ? 'case-card case-card-active' : 'case-card'} onClick={() => selectCase(record)}>
                    <span className="case-card-title">{record.matterName}</span>
                    <span className="case-card-meta">{record.clientName} • {record.matterType}</span>
                    <span className="case-card-footer">{record.status} • updated {formatDateTime(record.updatedAt)}</span>
                  </button>
                ))}
              </div>
            </div>
            <aside className="card">
              <div className="section-label">Selected case</div>
              <h2>{activeCase?.matterName || 'No case selected'}</h2>
              <p className="subtle small">{activeCase ? `${activeCase.clientName} • ${activeCase.matterType}` : 'Create a case from the intake form, then click it to open the workspace tabs.'}</p>
              {activeCase && <div className="intake-summary"><p><b>Issue:</b> {activeCase.issue || 'No issue entered.'}</p><p><b>Contact:</b> {activeCase.phone || activeCase.email || 'No contact entered.'}</p><p><b>Status:</b> {activeCase.status}</p></div>}
            </aside>
          </section>
          ...
          {activeCase && (
            ...
            {[
              ...
              <button key={key} className={caseSubTab === key ? 'case-tab active' : 'case-tab'} onClick={() => setCaseSubTab(key as any)}>{label}</button>
            ))}
            </div>
          )}

          {(!activeCase || caseSubTab === 'intake') && (
            <section className="grid">
              <div className="card wide-card">
                <div className="section-label">New client intake</div>
                <h2>Sample intake form</h2>
                <p className="subtle small">Collect essential background information before the firm provides service. This is the first case workspace screen so staff can start from intake, then move to documents, tasks, workflows, billing, and tools.</p>
                <div className="two-col">
                  <div><label className="small">Full name</label><input value={intakeForm.name} onChange={(event) => setIntakeForm({ ...intakeForm, name: event.target.value })} placeholder="Client name" /></div>
                  <div><label className="small">Phone</label><input value={intakeForm.phone} onChange={(event) => setIntakeForm({ ...intakeForm, phone: event.target.value })} placeholder="Phone number" /></div>
                  <div><label className="small">Email</label><input value={intakeForm.email} onChange={(event) => setIntakeForm({ ...intakeForm, email: event.target.value })} placeholder="Email address" /></div>
                  <div><label className="small">Matter type</label><select value={intakeForm.matterType} onChange={(event) => setIntakeForm({ ...intakeForm, matterType: event.target.value })}><option>General</option><option>Family Law</option><option>Immigration</option><option>Criminal</option><option>Civil</option></select></div>
                </div>
                <label className="small">Address</label><input value={intakeForm.address} onChange={(event) => setIntakeForm({ ...intakeForm, address: event.target.value })} placeholder="Street, city, state, ZIP" />
                <label className="small">Issue / reason for contact</label><textarea value={intakeForm.issue} onChange={(event) => setIntakeForm({ ...intakeForm, issue: event.target.value })} placeholder="Describe the client problem, deadline, court date, agency notice, or requested help." />
                <div className="two-col">
                  <div><label className="small">Preferred contact</label><select value={intakeForm.preferredContact} onChange={(event) => setIntakeForm({ ...intakeForm, preferredContact: event.target.value })}><option>Phone</option><option>Email</option><option>Text</option><option>Client portal</option></select></div>
                  <div><label className="small">Template</label><select><option>General intake</option><option>Family law intake</option><option>Immigration intake</option><option>Criminal intake</option></select></div>
                </div>
                <label className="small">Staff notes</label><textarea value={intakeForm.notes} onChange={(event) => setIntakeForm({ ...intakeForm, notes: event.target.value })} placeholder="Internal notes, conflict check issues, urgent facts, documents needed." />
                <div className="btn-row"><button onClick={saveIntakeAsCase}>Save intake to case workspace</button><button className="secondary" onClick={() => setCaseSubTab('workflows')}>Next: choose workflow</button></div>
              </div>
              <aside className="card">
                <div className="section-label">Intake summary</div>
                <h2>{intakeForm.name || 'New client'}</h2>
                <div className="intake-summary">
                  <p><b>Issue:</b> {intakeForm.issue || 'Not entered yet.'}</p>
                  <p><b>Contact:</b> {intakeForm.phone || intakeForm.email || 'Not entered yet.'}</p>
                  <p><b>Preferred:</b> {intakeForm.preferredContact}</p>
                  <p><b>Matter type:</b> {intakeForm.matterType}</p>
                </div>
                <p className="footer-note">Use this intake as the matter anchor for calendar events, tasks, document requests, and packet workflows.</p>
              </aside>
            </section>
          )}

          {activeCase && caseSubTab === 'info' && (
            <section className="grid">
              <div className="card">
                <div className="section-label">Case / matter</div>
                <h2>Create client + matter</h2>
                {Object.entries(matterForm).map(([key, value]) => (
                  <div key={key}><label className="small">{key}</label><input value={String(value)} onChange={(event) => setMatterForm((current) => ({ ...current, [key]: event.target.value }))} /></div>
                ))}
                <button onClick={createMatter} disabled={loading}>Create matter</button>
              </div>
              <div className="card">
                <div className="section-label">Existing cases</div>
                <h2>Local cases + database matters</h2>
                <div className="list">
                  {caseRecords.length === 0 && matters.length === 0 && <div className="item small subtle">No cases yet. Create one from Intake.</div>}
                  {caseRecords.map((record) => <div className="item" key={record.id}><div className="item-title">{record.matterName}</div><span className="badge">{record.status}</span><span className="badge">{record.matterType}</span><div className="small subtle">Client: {record.clientName}</div><button className="secondary mini" onClick={() => selectCase(record)}>Open case tabs</button></div>)}
                  {matters.map((matter) => <div className="item" key={matter.id}><div className="item-title">{matter.matter_name}</div><span className="badge">{matter.status}</span><span className="badge">{matter.matter_type}</span><div className="small subtle">Client: {matter.clients?.name || 'Unknown'}</div><button className="secondary mini" onClick={() => loadMatterIntoJson(matter)}>Use in workspace</button></div>)}
                </div>
              </div>
            </section>
          )}

          {activeCase && caseSubTab === 'calendar' && (
            <section className="card">
              <div className="section-label">Case calendar shortcut</div>
              <h2>Events tied to this case</h2>
              <p className="subtle">Calendar events can be attached to a client and matter. Use the main Calendar tab for the full month view, conflict checking, meeting links, briefs, and AI notes.</p>
              <div className="btn-row"><button onClick={() => setActiveTab('calendar')}>Open full calendar</button></div>
              <div className="list spaced">{calendarEvents.filter((event) => !matterName || event.matterName === matterName || event.clientName === intakeForm.name).slice(0, 6).map((event) => <div className="item" key={event.id}><div className="item-title">{event.title}</div><span className="badge">{event.date} {event.time}</span><span className="badge">{event.eventType}</span><p className="small preview">{event.brief || event.notes}</p></div>)}</div>
            </section>
          )}

          {activeCase && caseSubTab === 'documents' && (
            <section className="grid">
              <div className="card wide-card">
                <div className="section-label">Case documents</div>
                <h2>Upload source documents, evidence, and templates</h2>
                <p className="subtle small">Upload multiple case documents here. Choose <b>approved</b> to make a source usable by the chatbot, or set the doc type to <b>Template</b> for reusable drafting material.</p>
                <div className="dropzone" onDragOver={(event) => event.preventDefault()} onDrop={handleSourceDrop}>
                  <b>Drag & drop case files or templates</b>
                  <span>PDF, DOCX, TXT, CSV, images, and template text files. Multiple files are supported.</span>
                  <input type="file" multiple accept=".txt,.md,.markdown,.csv,.pdf,.docx,.png,.jpg,.jpeg,.webp,text/plain,text/markdown,text/csv,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/*" onChange={handleSourceFile} />
                </div>
                ...
                <div className="two-col">
                  <div><label className="small">Title</label><input value={sourceForm.title} onChange={(event) => setSourceForm({ ...sourceForm, title: event.target.value })} placeholder="Optional title" /></div>
                  <div><label className="small">Doc type</label><select value={sourceForm.doc_type} onChange={(event) => setSourceForm({ ...sourceForm, doc_type: event.target.value })}><option>Case evidence</option><option>Source document</option><option>Template</option><option>Checklist</option><option>Court order</option><option>Client upload</option></select></div>
                </div>
                <div className="two-col">
                  ...
                  <div><label className="small">Status</label><select value={sourceForm.status} onChange={(event) => setSourceForm({ ...sourceForm, status: event.target.value })}><option>approved</option><option>needs_review</option><option>do_not_use</option></select></div>
                </div>
                <label className="small">Paste template/source text if needed</label><textarea className="json-box" value={sourceForm.content} onChange={(event) => setSourceForm({ ...sourceForm, content: event.target.value })} placeholder="Paste template or source text here if you are not uploading a file." />
                <div className="btn-row"><button onClick={uploadSourceDocument} disabled={loading}>Upload document(s)</button><button className="secondary" onClick={() => setCaseSubTab('tools')}>Open packet/filler tools</button></div>
              </div>
              <div className="card">
                <div className="section-label">Case source library</div>
                <h2>Source documents</h2>
                <div className="list compact-list">{sourceDocs.slice(0, 8).map((doc) => <div className="item" key={doc.id}><div className="item-title">{doc.title}</div><span className="badge">{doc.status}</span><span className="badge">{doc.doc_type}</span><p className="small preview">{preview(doc.content)}</p></div>)}</div>
              </div>
              <div className="card">
                <div className="section-label">Template vault</div>
                <h2>Add a reusable template</h2>
                <label className="small">Template name</label><input value={templateForm.name} onChange={(event) => setTemplateForm({ ...templateForm, name: event.target.value })} />
                <label className="small">Template body</label><textarea value={templateForm.body_markdown} onChange={(event) => setTemplateForm({ ...templateForm, body_markdown: event.target.value })} placeholder="Paste a reusable legal template here." />
                <button onClick={createTemplate} disabled={loading}>Create template</button>
              </div>
              <div className="card">
                <div className="section-label">Recent generated documents</div>
                <h2>Drafts</h2>
                <div className="list compact-list">{generatedDocuments.slice(0, 5).map((doc) => <div className="item" key={doc.id}><div className="item-title">{doc.draft_type}</div><span className="badge">{doc.status}</span><p className="small preview">{preview(doc.output_markdown)}</p></div>)}</div>
              </div>
            </section>
          )}

          {activeCase && caseSubTab === 'tasks' && (
            <section className="card">
              <div className="section-label">Case tasks</div>
              <h2>Lawyer checklist + client homework</h2>
              <p className="subtle">Tasks can be attached to the case and split between the lawyer/staff checklist and the client checklist.</p>
              <button onClick={() => setActiveTab('tasks')}>Open task board</button>
            </section>
          )}

          {activeCase && caseSubTab === 'notes' && (
            <section className="card">
              <div className="section-label">Case notes</div>
              <h2>Notes</h2>
              <p className="subtle small">Notes are attached to the selected case. Use the microphone to dictate call notes, strategy notes, or attorney review notes, then review/edit before relying on them.</p>
              <div className="btn-row dense"><button className="secondary" type="button" onClick={startNotesVoice} disabled={isDictating}>{isDictating ? 'Listening...' : '🎙 Speak note'}</button></div>
              <textarea value={caseNote} onChange={(event) => { const value = event.target.value; setCaseNote(value); if (activeCase) setCaseRecords((current) => current.map((item) => item.id === activeCase.id ? { ...item, notes: value, updatedAt: new Date().toISOString() } : item)); }} placeholder="Internal case notes, call summaries, strategy notes, attorney review notes." />
              <p className="footer-note">These notes stay tied to this case record and can be copied into a task, event, or generated packet.</p>
            </section>
          )}

          {activeCase && caseSubTab === 'billing' && (
            <section className="card"><div className="section-label">Case billing</div><h2>Billing placeholder</h2><p className="subtle">Track fee agreements, invoices, payment status, trust items, and time entries here. The main Billing tab has the firm-wide view.</p><button onClick={() => setActiveTab('billing')}>Open billing</button></section>
          )}

          {activeCase && caseSubTab === 'workflows' && (
            <section className="grid">
              <div className="card">
                <div className="section-label">Workflow templates</div>
                <h2>Apply workflow to case</h2>
                <p className="subtle small">Workflows hold reusable lawyer checklists and client homework checklists. Choose one, generate the steps, then edit tasks as needed.</p>
                <label className="small">Workflow</label><select value={workflowChoice} onChange={(event) => setWorkflowChoice(event.target.value)}>{WORKFLOW_PRESETS.map((workflow) => <option key={workflow.name}>{workflow.name}</option>)}</select>
                <p className="preview">{WORKFLOW_PRESETS.find((workflow) => workflow.name === workflowChoice)?.description}</p>
                <button onClick={() => addWorkflowTasksFromPreset(workflowChoice, intakeForm.name || 'Client', matterName || intakeForm.issue || intakeForm.matterType)}>Generate workflow tasks</button>
              </div>
              <div className="card">
                <div className="section-label">Create workflow</div>
                <h2>Type or speak steps</h2>
                <textarea value={customWorkflowText} onChange={(event) => setCustomWorkflowText(event.target.value)} placeholder={`Type each step on its own line. Example:
Client uploads police report
Attorney reviews discovery
Paralegal prepares evidence packet`} />
                <div className="btn-row"><button className="secondary" onClick={startWorkflowVoice} disabled={isDictating}>{isDictating ? 'Listening...' : 'Speak workflow step'}</button><button onClick={addCustomWorkflowTasks}>Create editable tasks</button></div>
              </div>
            </section>
          )}

          {activeCase && caseSubTab === 'tools' && (
            <section className="grid">
              <div className="card"><div className="section-label">Documents → Tools</div><h2>Packet Builder</h2><p className="subtle">Build motions, responses, replies, exhibit indexes, TOCs, and proposed orders from uploaded exhibits.</p><button onClick={() => setActiveTab('packet')}>Open Packet Builder</button></div>
              <div className="card"><div className="section-label">Documents → Tools</div><h2>Document Filler</h2><p className="subtle">Analyze and fill PDF/DOCX forms through a guided workflow.</p><button onClick={() => setActiveTab('filler')}>Open Document Filler</button></div>
              <div className="card"><div className="section-label">Client homework</div><h2>Checklists</h2><p className="subtle">Send client and attorney checklists from uploaded checklist templates.</p><button onClick={() => setActiveTab('checklists')}>Open Checklists</button></div>
            </section>
          )}
        </section>
      )}
      ...
      {activeTab === 'billing' && (
        <section className="grid">
          <div className="card"><div className="section-label">Billing</div><h2>Firm billing overview</h2><p className="subtle">Placeholder for invoices, payment status, fee agreements, time entries, and account balance. This keeps billing visible as a main tab while detailed case billing can sit inside Cases.</p><div className="metric-grid"><div className="metric"><b>$0</b><span>open balance demo</span></div><div className="metric"><b>0</b><span>pending invoices</span></div></div></div>
          <div className="card"><div className="section-label">Next build</div><h2>Billing integrations</h2><p className="subtle">Connect Stripe, QuickBooks, or firm accounting later. For now this tab shows the workspace location and structure.</p></div>
        </section>
      )}
      ...
      {activeTab === 'admin' && canManageUsers && (
        <section className="admin-workspace">
          <div className="case-tabs">
            <button className={adminSubTab === 'source' ? 'case-tab active' : 'case-tab'} onClick={() => setAdminSubTab('source')}>Source Files</button>
            <button className={adminSubTab === 'users' ? 'case-tab active' : 'case-tab'} onClick={() => setAdminSubTab('users')}>Users</button>
            <button className={adminSubTab === 'audit' ? 'case-tab active' : 'case-tab'} onClick={() => setAdminSubTab('audit')}>Audit</button>
          </div>
          {adminSubTab === 'source' && (
            <section className="grid"><div className="card"><div className="section-label">Admin source files</div><h2>Upload source files</h2><p className="subtle small">Admins can upload the firm knowledge base here. Testers do not see this tab.</p><div className="dropzone" onDragOver={(event) => event.preventDefault()} onDrop={handleSourceDrop}><b>Drag & drop multiple source files</b><span>PDF, DOCX, TXT, CSV, or images.</span><input type="file" multiple onChange={handleSourceFile} /></div><button onClick={uploadSourceDocument} disabled={loading}>Upload source document(s)</button></div><div className="card"><h2>Approved source files</h2><div className="list compact-list">{sourceDocs.map((doc) => <div className="item" key={doc.id}><div className="item-title">{doc.title}</div><span className="badge">{doc.status}</span><p className="small preview">{preview(doc.content)}</p></div>)}</div></div></section>
          )}
          {adminSubTab === 'users' && (
            <section className="grid"><div className="card"><div className="section-label">Admin users</div><h2>Create user</h2><label className="small">Email</label><input value={userForm.email} onChange={(event) => setUserForm({ ...userForm, email: event.target.value })} /><label className="small">Temporary password</label><input type="password" value={userForm.password} onChange={(event) => setUserForm({ ...userForm, password: event.target.value })} /><label className="small">Full name</label><input value={userForm.full_name} onChange={(event) => setUserForm({ ...userForm, full_name: event.target.value, display_name: event.target.value })} /><div className="two-col"><div><label className="small">Role</label><select value={userForm.role} onChange={(event) => setUserForm({ ...userForm, role: event.target.value })}><option>owner</option><option>admin</option><option>attorney</option><option>paralegal</option><option>intake</option><option>viewer</option><option>client</option></select></div><div><label className="small">Status</label><select value={userForm.status} onChange={(event) => setUserForm({ ...userForm, status: event.target.value })}><option>active</option><option>pending</option><option>disabled</option></select></div></div><label className="small">Chatbot behavior</label><textarea value={userForm.chatbot_persona} onChange={(event) => setUserForm({ ...userForm, chatbot_persona: event.target.value })} /><button onClick={createFirmUser} disabled={loading}>Create / update user</button></div><div className="card"><div className="section-label">All users</div><h2>Accounts</h2><div className="list compact-list">{firmUsers.map((firmUser) => <div className="item" key={firmUser.user_id}><div className="item-title">{firmUser.display_name || firmUser.email}</div><span className="badge">{firmUser.role}</span><span className="badge">{firmUser.status}</span><p className="small subtle">{firmUser.email}</p><div className="btn-row dense"><button className="secondary mini" onClick={() => updateFirmUser(firmUser, { role: 'admin' })}>Make admin</button><button className="secondary mini" onClick={() => updateFirmUser(firmUser, { role: 'attorney' })}>Make attorney</button><button className="secondary mini danger-button" onClick={() => disableUserWithConfirm(firmUser)}>Disable account</button></div></div>)}</div></div></section>
          )}
          {adminSubTab === 'audit' && (
            <section className="grid"><div className="card"><div className="section-label">Admin audit</div><h2>Activity and chat visibility</h2><p className="subtle small">Admins can review user activity, generated work, and chat conversation metadata. Full chat message visibility depends on the database/API permissions.</p><div className="list compact-list">{logs.map((log) => <div className="item" key={log.id}><div className="item-title">{log.action}</div><p className="small subtle">{log.actor_email || 'system'} • {formatDate(log.created_at)}</p><p className="small preview">{log.output_preview}</p></div>)}</div></div><div className="card"><div className="section-label">Chat questions</div><h2>Recent chats by user</h2><div className="list compact-list">{chatConversations.map((chat) => <div className="item" key={chat.id}><div className="item-title">{conversationLabel(chat)}</div><p className="small subtle">{chat.created_by_email || 'unknown'} • {formatDate(chat.updated_at)}</p></div>)}</div></div></section>
          )}
        </section>
      )}
          ...
          <section className="card chat-card">
            ...
            <div className="chat-composer">
              ...
              <div className="chat-composer-actions">
                <button className="secondary" type="button" onClick={startChatVoice} disabled={loading || isDictating}>🎙 {isDictating ? 'Listening...' : 'Speak'}</button>
                <button onClick={sendChatMessage} disabled={loading || (!chatInput.trim() && !chatFiles.length)}>Send</button>
              </div>
            </div>
            <p className="footer-note">Tip: press Enter to send. Press Shift+Enter for a new line. Web/general answers are not firm-approved legal advice.</p>
          </section>
        </section>
      )}
      ...
      {activeTab === 'calendar' && (
        <section className="calendar-workspace enhanced-calendar-workspace">
          <div className="calendar-main card">
            <div className="calendar-toolbar">
              <div>
                <div className="section-label">Firm-wide calendar</div>
                <h2>{calendarMonthTitle}</h2>
                <p className="subtle small">Shows all activities for all users. Filter by user/event type, attach events to matters, detect conflicts, and generate deadline sets.</p>
              </div>
            ...
            <div className="calendar-filter-bar">
              <label><span>Users</span><select value={calendarUserFilter} onChange={(event) => setCalendarUserFilter(event.target.value)}><option value="all">All users</option>{firmUsers.map((user) => <option key={user.user_id} value={user.user_id}>{user.display_name || user.email}</option>)}</select></label>
              <label><span>Event types</span><select value={calendarEventTypeFilter} onChange={(event) => setCalendarEventTypeFilter(event.target.value)}><option value="all">All event types</option>{EVENT_TYPES.map((type) => <option key={type.name}>{type.name}</option>)}</select></label>
              <div className="event-type-legend">{EVENT_TYPES.slice(0, 9).map((type) => <span key={type.name}><i style={{ background: type.color }} />{type.name}</span>)}</div>
            </div>
                  ...
                  >
                    ...
                    <div className="calendar-day-events">
                      {dayEvents.slice(0, 5).map((item) => (
                        <span className="calendar-event-pill typed-event-pill" style={{ borderLeftColor: item.color || eventTypeColor(item.eventType) }} key={item.id} onClick={(event) => { event.stopPropagation(); setSelectedCalendarEventId(item.id); }}>
                          {item.time ? `${item.time} ` : ''}{item.title}
                        </span>
                      ))}
                      {dayEvents.length > 5 && <span className="calendar-more">+{dayEvents.length - 5} more</span>}
                    </div>
                  </button>
                );
              })}
          ...
          <aside className="calendar-side enhanced-calendar-side">
            <div className="card">
              <div className="section-label">Add event</div>
              <h2>{shortDate(selectedCalendarDate)}</h2>
              <label className="small">Case or matter</label><input value={calendarForm.matterName} onChange={(event) => setCalendarForm({ ...calendarForm, matterName: event.target.value })} placeholder="Matter attached to a client" />
              <label className="small">Client</label><input value={calendarForm.clientName} onChange={(event) => setCalendarForm({ ...calendarForm, clientName: event.target.value })} placeholder="Client name" />
              <label className="small">Event name</label><input value={calendarForm.title} onChange={(event) => setCalendarForm({ ...calendarForm, title: event.target.value })} placeholder="Hearing, filing deadline, client call..." />
              <div className="two-col">
                ...
                <div><label className="small">Type</label><select value={calendarForm.eventType} onChange={(event) => setCalendarForm({ ...calendarForm, eventType: event.target.value })}>{EVENT_TYPES.map((type) => <option key={type.name}>{type.name}</option>)}</select></div>
              </div>
              <div className="two-col">
                <div><label className="small">Start</label><input type="time" value={calendarForm.time} onChange={(event) => setCalendarForm({ ...calendarForm, time: event.target.value })} /></div>
                <div><label className="small">End</label><input type="time" value={calendarForm.endTime} onChange={(event) => setCalendarForm({ ...calendarForm, endTime: event.target.value })} /></div>
              </div>
              <label className="small">Assign user</label><select value={calendarForm.assignedUserId} onChange={(event) => setCalendarForm({ ...calendarForm, assignedUserId: event.target.value })}><option value="">Current / unassigned</option>{firmUsers.map((user) => <option key={user.user_id} value={user.user_id}>{user.display_name || user.email}</option>)}</select>
              <label className="small">Location</label><input value={calendarForm.location} onChange={(event) => setCalendarForm({ ...calendarForm, location: event.target.value })} placeholder="Court, office, jail, Zoom, USCIS office..." />
              <label className="small">Meeting link</label><input value={calendarForm.meetingLink} onChange={(event) => setCalendarForm({ ...calendarForm, meetingLink: event.target.value })} placeholder="Zoom/Meet/Teams link" />
              <label className="small">Brief on the event</label><textarea value={calendarForm.brief} onChange={(event) => setCalendarForm({ ...calendarForm, brief: event.target.value })} placeholder="Short brief that appears when the event is clicked." />
              <label className="small">AI generated notes</label><textarea value={calendarForm.aiNotes} onChange={(event) => setCalendarForm({ ...calendarForm, aiNotes: event.target.value })} placeholder="AI notes, strategy reminders, deadline cautions, and next steps." />
              <label className="small">Description</label><textarea value={calendarForm.notes} onChange={(event) => setCalendarForm({ ...calendarForm, notes: event.target.value })} placeholder="Who attends, what to bring, filings due, client instructions..." />
              ...
              <label className="checkline"><input type="checkbox" checked={calendarForm.addTrialWorkflow} onChange={(event) => setCalendarForm({ ...calendarForm, addTrialWorkflow: event.target.checked })} /> Add linked trial deadline set: trial, evidence due, client evidence due, prep call</label>
              <button onClick={addCalendarEvent} disabled={loading}>Add event</button>
            </div>
            ...
            {selectedCalendarEvent && (
              <div className="card event-detail-card">
                ...
                <p className="small subtle">{selectedCalendarEvent.date} {selectedCalendarEvent.time || ''}{selectedCalendarEvent.endTime ? ` – ${selectedCalendarEvent.endTime}` : ''}</p>
                <span className="badge">{selectedCalendarEvent.eventType || 'Event'}</span>
                <span className="badge">{selectedCalendarEvent.assignedUserName || assignedUserLabel(selectedCalendarEvent.assignedUserId)}</span>
                {selectedCalendarEvent.matterName && <p><b>Matter:</b> {selectedCalendarEvent.matterName}</p>}
                {selectedCalendarEvent.clientName && <p><b>Client:</b> {selectedCalendarEvent.clientName}</p>}
                {selectedCalendarEvent.location && <p><b>Location:</b> {selectedCalendarEvent.location}</p>}
                {selectedCalendarEvent.meetingLink && <p><b>Meeting link:</b> <a href={selectedCalendarEvent.meetingLink} target="_blank" rel="noreferrer">Open link</a></p>}
                {selectedCalendarEvent.brief && <p><b>Brief:</b> {selectedCalendarEvent.brief}</p>}
                {selectedCalendarEvent.aiNotes && <p><b>AI notes:</b> {selectedCalendarEvent.aiNotes}</p>}
                {selectedCalendarEvent.notes && <p><b>Description:</b> {selectedCalendarEvent.notes}</p>}
                <div className="btn-row dense"><button className="secondary mini" onClick={() => removeCalendarEvent(selectedCalendarEvent.id)}>Delete event</button><button className="secondary mini" onClick={() => setSelectedCalendarEventId('')}>Close</button></div>
              </div>
            )}
            ...
            <div className="card">
              ...
              <div className="list calendar-event-list">
                ...
                {selectedCalendarEvents.map((item) => (
                  <button className="item event-list-button" key={item.id} onClick={() => setSelectedCalendarEventId(item.id)}>
                    <div className="item-title">{item.title}</div>
                    <span className="badge">{item.time || 'No time'}</span><span className="badge">{item.eventType || 'Event'}</span>
                    <p className="small preview">{item.brief || item.notes || item.matterName}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="list calendar-event-list">
                ...
                {upcomingCalendarEvents.map((item) => <button className="item event-list-button" key={item.id} onClick={() => { setSelectedCalendarEventId(item.id); selectCalendarDate(item.date); }}><div className="item-title">{item.title}</div><span className="badge">{item.date} {item.time || ''}</span><p className="small preview">{item.brief || item.notes}</p></button>)}
              </div>
            </div>
          </aside>
        </section>
      ...
      {activeTab === 'tasks' && (
        <section className="task-board-layout">
          <div className="card">
            <div className="section-label">Tasks</div>
            <h2>Add task + reminder</h2>
            <p className="subtle small">Create lawyer/staff tasks and client homework. Tasks can be attached to a case and tied to workflows.</p>
            ...
            <div className="two-col">
              <div><label className="small">Case / lead</label><input value={taskForm.caseName} onChange={(event) => setTaskForm({ ...taskForm, caseName: event.target.value })} placeholder="Case or matter" /></div>
              <div><label className="small">Client</label><input value={taskForm.clientName} onChange={(event) => setTaskForm({ ...taskForm, clientName: event.target.value })} placeholder="Client name" /></div>
              <div><label className="small">Assigned to</label><input value={taskForm.assignedTo} onChange={(event) => setTaskForm({ ...taskForm, assignedTo: event.target.value })} placeholder="Staff/user/client" /></div>
              <div><label className="small">Checklist type</label><select value={taskForm.ownerType} onChange={(event) => setTaskForm({ ...taskForm, ownerType: event.target.value as 'lawyer' | 'client' })}><option value="lawyer">Lawyer / staff checklist</option><option value="client">Client homework checklist</option></select></div>
              ...
              <div><label className="small">Priority</label><select value={taskForm.priority} onChange={(event) => setTaskForm({ ...taskForm, priority: event.target.value as 'Low' | 'Medium' | 'High' })}><option>Low</option><option>Medium</option><option>High</option></select></div>
            </div>
            <label className="small">Reminder</label><select value={taskForm.reminder} onChange={(event) => setTaskForm({ ...taskForm, reminder: event.target.value })}><option>At due date</option><option>1 day before</option><option>3 days before</option><option>1 week before</option></select>
            <label className="small">Workflow</label><select value={taskForm.workflowName} onChange={(event) => setTaskForm({ ...taskForm, workflowName: event.target.value })}><option value="">No workflow</option>{WORKFLOW_PRESETS.map((workflow) => <option key={workflow.name}>{workflow.name}</option>)}</select>
            <label className="small">Notes / comments</label><textarea value={taskForm.notes} onChange={(event) => setTaskForm({ ...taskForm, notes: event.target.value })} />
            <button onClick={addTask} disabled={loading}>Add task</button>
            <div className="divider" />
            <h2>Apply workflow</h2>
            <p className="subtle small">Workflows generate both lawyer checklists and client homework checklists. They can be edited after creation.</p>
            <label className="small">Workflow</label><select value={workflowChoice} onChange={(event) => setWorkflowChoice(event.target.value)}>{WORKFLOW_PRESETS.map((workflow) => <option key={workflow.name}>{workflow.name}</option>)}</select>
            <button className="secondary" onClick={() => addWorkflowTasksFromPreset(workflowChoice, intakeForm.name || 'Client', matterName || intakeForm.issue || 'Case')}>Generate workflow tasks</button>
          </div>

          <div className="card task-board-main">
            ...
            <div className="task-columns">
              <div>
                <h3>Lawyer / staff checklist</h3>
                <div className="list spaced">
                  {tasks.filter((item) => item.ownerType !== 'client').length === 0 && <div className="item small subtle">No lawyer tasks yet.</div>}
                  {tasks.filter((item) => item.ownerType !== 'client').map((item) => (
                    <div className={`item task-item priority-${(item.priority || 'Medium').toLowerCase()}`} key={item.id}>
                      <div className="item-title">{item.title}</div>
                      <span className="badge">{item.status}</span><span className="badge">{item.priority || 'Medium'}</span><span className="badge">Due: {item.dueDate || 'No due date'}</span>
                      {item.caseName && <span className="badge">{item.caseName}</span>}
                      {item.assignedTo && <p className="small subtle">Assigned to: {item.assignedTo}</p>}
                      {item.notes && <p className="small preview">{item.notes}</p>}
                      <div className="btn-row dense"><button className="secondary mini" onClick={() => updateTaskStatus(item.id, item.status === 'done' ? 'open' : 'done')}>{item.status === 'done' ? 'Reopen' : 'Mark done'}</button><button className="secondary mini" onClick={() => removeTask(item.id)}>Remove</button></div>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <h3>Client homework checklist</h3>
                <div className="list spaced">
                  {tasks.filter((item) => item.ownerType === 'client').length === 0 && <div className="item small subtle">No client homework yet.</div>}
                  {tasks.filter((item) => item.ownerType === 'client').map((item) => (
                    <div className={`item task-item priority-${(item.priority || 'Medium').toLowerCase()}`} key={item.id}>
                      <div className="item-title">{item.title}</div>
                      <span className="badge">{item.status}</span><span className="badge">Client</span><span className="badge">Due: {item.dueDate || 'No due date'}</span>
                      {item.clientName && <p className="small subtle">Client: {item.clientName}</p>}
                      {item.notes && <p className="small preview">{item.notes}</p>}
                      <p className="footer-note">Email can include the homework list. SMS should stay vague: “You have items to complete in First Chair.”</p>
                      <div className="btn-row dense"><button className="secondary mini" onClick={() => updateTaskStatus(item.id, item.status === 'done' ? 'open' : 'done')}>{item.status === 'done' ? 'Reopen' : 'Mark done'}</button><button className="secondary mini" onClick={() => removeTask(item.id)}>Remove</button></div>
                    </div>
                  ))}
                </div>
